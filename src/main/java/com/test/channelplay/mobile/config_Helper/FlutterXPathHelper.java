...
public class FlutterXPathHelper {
    ...
    private String findBestTemplate(String fieldName, String manualTemplate) {
        ...
        // Priority 1: Check manual captures (manual_captured_images folder)
        ...
        // Priority 2: Check XPath captures (screens folder)
        for (String template : templates) {
        ...
        // Priority 3: Check AI captures (AI_images folder)
        for (String template : templates) {
    ...
    private WebElement findFocusedElement() {
        ...
        // Try each strategy in order (prioritize EditText to avoid Flutter wrapper issues)
        element = findActiveElement();
        if (element != null) return element;

        // Prioritize EditText search before generic focused attribute
        // This prevents Flutter from returning wrapper/container elements
        element = findFocusedEditText();
        if (element != null) return element;

        // Strategy: Find EditText by companion 'T' button (text formatting/keyboard toggle)
        element = findEditTextByCompanionButton();
        if (element != null) return element;
    ...
    //  *** Helper method of findFocusedElement()
    private WebElement findEditTextByCompanionButton() {
        try {
            // Flutter text fields often have a companion 'T' button (text formatting/keyboard toggle)
            // This button is clickable and sits to the right of the EditText
            // We can use it to identify and locate the EditText field

            // Find all clickable ImageView or View elements (potential 'T' buttons)
            List<WebElement> clickableButtons = driver.findElements(AppiumBy.xpath(
                "//android.widget.ImageView[@clickable='true'] | //android.view.View[@clickable='true']"
            ));

            log.debug("Companion button strategy: Found {} clickable buttons to check", clickableButtons.size());

            // For each button, check if there's an EditText nearby (preceding sibling or same parent)
            for (WebElement button : clickableButtons) {
                try {
                    // Strategy 1: Find EditText as preceding sibling of this button
                    List<WebElement> siblingEditTexts = driver.findElements(AppiumBy.xpath(
                        "//android.widget.EditText[following-sibling::*[contains(@class,'ImageView') or contains(@class,'View')][@clickable='true']]"
                    ));

                    for (WebElement editText : siblingEditTexts) {
                        if (editText.isDisplayed() && editText.isEnabled()) {
                            // Check if this EditText is in focus state
                            String focused = editText.getAttribute("focused");
                            if ("true".equals(focused)) {
                                log.info("Found focused EditText via companion button strategy");
                                return editText;
                            }
                        }
                    }
                    ...
                    log.debug("Companion button check failed for one button: {}", e.getMessage());
                }
            }

            log.debug("Companion button strategy: No focused EditText found");
        } catch (Exception e) {
            log.debug("Companion button strategy failed: {}", e.getMessage());
        }
        return null;
    }
    ...
    private WebElement findFocusedEditText() {
        // Try with retry - Flutter may need a moment to update focus state
        for (int attempt = 0; attempt < 3; attempt++) {
            try {
                List <WebElement> editTexts = driver.findElements(AppiumBy.xpath("//android.widget.EditText"));
                log.debug("Found {} EditText elements on screen (attempt {})", editTexts.size(), attempt + 1);

                for (WebElement editText : editTexts) {
                    if (isElementFocused(editText)) {
                        log.info("Found focused EditText via attribute check on attempt {}", attempt + 1);
                        return editText;
                    }
                }

                // If no focused EditText found, wait briefly and retry
                if (attempt < 2) {
                    Thread.sleep(200);  // Wait 200ms for Flutter to update focus
                }
            } catch (Exception e) {
                log.debug("EditText focused search failed on attempt {}: {}", attempt + 1, e.getMessage());
            }
        }

        log.debug("No focused EditText found after 3 attempts");
        return null;
    }
    ...
    private WebElement findLastResortEditText() {
        try {
            // Find all EditText elements (not just clickable or last one)
            List<WebElement> editTexts = driver.findElements(AppiumBy.xpath("//android.widget.EditText"));
            log.debug("Last resort: Found {} EditText elements to check", editTexts.size());

            // Return first visible and enabled EditText
            for (WebElement editText : editTexts) {
                if (editText.isDisplayed() && editText.isEnabled()) {
                    log.info("Last resort: Found visible and enabled EditText");
                    return editText;
                }
            }

            log.debug("Last resort: No visible and enabled EditText found");
        } catch (Exception e) {
     ...
     * @param folderPath The folder path relative to project root (e.g., "screenshots/element_interaction_failures_scenarioTag")
    ...
    /**
     * Save element interaction failure screenshot with standardized folder structure
     *
     * Automatically constructs folder path: screenshots/element_interaction_failures_[scenarioTag]
     * This provides a consistent folder naming convention for all interaction failure screenshots
     * across different test scenarios.
     *
     * @param filename The name of the screenshot file (e.g., "description_error.png")
     * @param scenarioTag The scenario tag to identify the failure context (e.g., "offsiteActivity", "linkedFields", "bulkUpload")
     * @return The full path where screenshot was saved
     */
    public String saveElementInteractionFailureScreenshot(String filename, String scenarioTag) {
        // Construct standardized folder path
        String folderPath = "screenshots/element_interaction_failures_" + scenarioTag;

        log.info("Saving interaction failure screenshot for scenario tag: {}", scenarioTag);

        // Delegate to existing method
        return saveScreenshotToFolder(filename, folderPath);
    }
